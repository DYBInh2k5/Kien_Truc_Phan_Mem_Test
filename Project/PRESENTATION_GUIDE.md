...
Tài liệu này tổng hợp toàn bộ nội dung thuyết trình, kịch bản nói chi tiết cho từng Slide, kịch bản demo thực tế trên giao diện, và bộ câu hỏi phản biện chuyên sâu kèm gợi ý trả lời dựa trên mã nguồn thật để nhóm sinh viên đạt kết quả cao nhất trước Hội đồng chấm thi.
...
*   **Định vị mã nguồn thực tế**:
    *   [Lớp DatabaseConnection](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/singleton.py#L6-L25) trong [singleton.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/singleton.py)
*   **🎤 Kịch bản nói (Speech Script)**:
    > "Mẫu thiết kế đầu tiên nhóm áp dụng là **Singleton Pattern** cho lớp quản lý kết nối cơ sở dữ liệu `DatabaseConnection` cài đặt tại file `singleton.py`. Trong môi trường Web API, có hàng trăm yêu cầu đọc/ghi dữ liệu gửi tới đồng thời. Nếu mỗi request lại tạo ra một kết nối SQLite mới, tài nguyên hệ thống sẽ nhanh chóng bị cạn kiệt và xảy ra lỗi 'Database is locked' của SQLite. Với Singleton kết hợp cơ chế kiểm tra kép Double-Checked Locking và Thread-lock, chúng em đảm bảo toàn bộ hệ thống chỉ duy trì duy nhất một kết nối an toàn trong suốt vòng đời chạy ứng dụng."
...
*   **Định vị mã nguồn thực tế**:
    *   [Lớp OrderFactory](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/factory.py#L33-L44) trong [factory.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/factory.py)
*   **🎤 Kịch bản nói (Speech Script)**:
    > "Tiếp theo là **Factory Method Pattern** ở file `factory.py`. Khi đặt hàng, phí vận chuyển và quy trình sẽ phụ thuộc vào phương thức giao hàng mà người dùng chọn (Giao hàng thường hoặc giao hỏa tốc). Chúng em đã định nghĩa một lớp cha trừu tượng là `Order`, và các lớp con cụ thể như `StandardOrder` và `ExpressOrder`. Lớp `OrderFactory` đóng vai trò là nhà máy sản xuất, tự động khởi tạo đúng đối tượng đơn hàng tương ứng. Nhờ mẫu thiết kế này, nếu trong tương lai doanh nghiệp muốn mở rộng thêm phương thức giao hàng mới như giao hàng bằng máy bay hay drone, chúng em chỉ cần viết thêm lớp con kế thừa mà hoàn toàn không cần sửa đổi mã nguồn ở tầng Controller."
...
*   **Định vị mã nguồn thực tế**:
    *   [Lớp OrderFacade](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/facade.py#L23-L70) trong [facade.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/facade.py)
*   **🎤 Kịch bản nói (Speech Script)**:
    > "Để đặt một đơn hàng thành công, hệ thống phải trải qua rất nhiều bước phức tạp: kiểm tra số lượng tồn kho của sản phẩm, thực hiện thanh toán chi phí đơn hàng, và điều phối vận đơn cho đơn vị vận chuyển. Thay vì để Controller giao tiếp trực tiếp với cả 3 phân hệ phức tạp này, chúng em áp dụng **Facade Pattern** thông qua lớp `OrderFacade` tại file `facade.py`. Lớp này cung cấp một giao diện cực kỳ đơn giản là `place_order()`. Controller chỉ cần gọi hàm này, mọi logic nghiệp vụ phức tạp bên dưới sẽ được Facade âm thầm điều phối. Điều này giúp giảm độ phụ thuộc chéo (coupling) và làm mã nguồn ở Controller vô cùng gọn gàng."
...
*   **Định vị mã nguồn thực tế**:
    *   [Lớp OrderContext](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/state.py#L36-L53) và các trạng thái trong [state.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/state.py)
*   **🎤 Kịch bản nói (Speech Script)**:
    > "Quản lý vòng đời trạng thái đơn hàng luôn là một thách thức lớn. Thông thường, lập trình viên sẽ viết các hàm cập nhật trạng thái chứa đầy các câu lệnh `if/else` để kiểm tra điều kiện chuyển đổi. Nhóm chúng em đã giải quyết triệt để vấn đề này bằng cách áp dụng **State Pattern** ở file `state.py`. Trạng thái của đơn hàng được đóng gói thành các đối tượng riêng biệt như `PendingState`, `PaidState` và `ShippedState`. Lớp ngữ cảnh `OrderContext` chỉ cần gọi phương thức `proceed()`, đơn hàng sẽ tự động nâng cấp trạng thái một cách tuần tự và thông minh dựa trên logic đóng gói sẵn trong từng lớp trạng thái, giúp loại bỏ hoàn toàn spaghetti code."
...
*   **Định vị mã nguồn thực tế**:
    *   [Lớp OrderCollection](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/iterator.py#L3-L31) trong [iterator.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/iterator.py)
*   **🎤 Kịch bản nói (Speech Script)**:
    > "Mẫu thiết kế cuối cùng là **Iterator Pattern**, được cài đặt cho lớp tập hợp `OrderCollection` trong file `iterator.py`. Khi truy vấn danh sách đơn hàng từ cơ sở dữ liệu SQLite, dữ liệu được nạp vào đối tượng `OrderCollection`. Nhờ triển khai Iterator, tầng Controller có thể duyệt qua danh sách đơn hàng bằng vòng lặp `for...in` chuẩn mực hoặc thực hiện tìm kiếm đơn hàng thông qua hàm `find_order()`. Mẫu thiết kế này giúp che giấu cấu trúc dữ liệu lưu trữ nội bộ của collection, giúp cho việc thay đổi cấu trúc lưu trữ sau này (từ mảng sang cây hoặc bảng băm) hoàn toàn không ảnh hưởng đến mã nguồn sử dụng bên ngoài."
...
*   **Nội dung hiển thị trên Slide**:
    ...
    *   *Cơ chế Fallback (Dự phòng)*:
        *   Nếu Microservices C# **Online**: Gọi C# để xác thực và lấy báo cáo thống kê chính xác.
        *   Nếu Microservices C# **Offline**: Tự động chuyển vùng dữ liệu sang SQLite cục bộ và trả về dữ liệu mock (đối với báo cáo), giúp hệ thống duy trì hoạt động 100% thời gian.
*   **Định vị mã nguồn thực tế**:
    *   [Hàm login() của AuthService](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/services/auth_service.py#L11-L66) trong [auth_service.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/services/auth_service.py)
    *   [Hàm get_csharp_report() của api_router](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/controllers/api_router.py#L97-L125) trong [api_router.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/controllers/api_router.py)
*   **🎤 Kịch bản nói (Speech Script)**:
...
*   **Định vị mã nguồn thực tế**:
    *   C# SSO: [Program.cs (SSOService)](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/microservices_csharp/SSOService/Program.cs)
    *   C# Search: [Program.cs (SearchService)](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/microservices_csharp/SearchService/Program.cs)
    *   C# Report: [Program.cs (ReportService)](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/microservices_csharp/ReportService/Program.cs)
*   **🎤 Kịch bản nói (Speech Script)**:
...
Hãy tuân thủ kịch bản thao tác và lời thuyết minh từng bước dưới đây:
...
3.  **Lấy báo cáo chéo từ C# Microservice**:
    ...
    *   *Giải thích*: *"Khi nhấn nút này, FastAPI đóng vai trò là một Proxy gửi yêu cầu tới C# Report Service trên cổng 5003. Số liệu thống kê được nạp và hiển thị trực tiếp lên Frontend."*
...
*   **Gợi ý trả lời**:
    ...
    >
    > Cơ chế **Double-Checked Locking** cài đặt tại [Lớp DatabaseConnection ở singleton.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/singleton.py#L17-L25) thực hiện kiểm tra biến `_instance` 2 lần: Lần 1 không dùng khóa Lock để tránh nghẽn hiệu năng của mọi luồng khi instance đã được tạo. Lần 2 nằm bên trong khối Lock `with cls._lock` để đảm bảo chắc chắn chỉ có duy nhất luồng đầu tiên chiếm được khóa mới được quyền khởi tạo đối tượng kết nối DB. Các luồng sau khi chờ khóa xong sẽ thấy instance đã được tạo ở lần kiểm tra thứ 2 và bỏ qua."
...
*   **Gợi ý trả lời**:
    > "Nhờ cấu trúc Loose Coupling của Factory Method cài đặt ở [factory.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/factory.py), việc mở rộng rất dễ dàng:
    > 1. Chúng em viết thêm một lớp con mới là `SuperExpressOrder` kế thừa từ lớp trừu tượng `Order` và ghi đè (override) phương thức `get_shipping_cost()` trả về giá trị `50.0`.
    > 2. Trong lớp `OrderFactory` tại phương thức [create_order()](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/factory.py#L38-L44), thêm một điều kiện kiểm tra chuỗi đầu vào: `elif order_type.lower() == 'superexpress': return SuperExpressOrder(product_id)`.
...
*   **Gợi ý trả lời**:
    ...
    > Ở chức năng đặt hàng, tầng Controller chỉ cần gọi thực thi chuỗi đặt hàng một chiều xuống các phân hệ Kho, Thanh toán, Vận chuyển mà các phân hệ này không cần giao tiếp ngược lại với Controller. Do đó, áp dụng **Facade Pattern** cài đặt tại [facade.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/facade.py) là giải pháp tối giản và phù hợp nhất."
...
*   **Gợi ý trả lời**:
    > "Nếu xử lý bằng `if/else` ngay tại lớp `Order`, khi vòng đời đơn hàng trở nên phức tạp (Ví dụ: thêm trạng thái 'Giao thất bại', 'Khách trả hàng', 'Hoàn tiền'), lớp `Order` sẽ nhanh chóng biến thành một 'God Object' chứa hàng trăm dòng code if/else lồng nhau phức tạp, cực kỳ khó đọc và dễ phát sinh lỗi khi sửa đổi.
    >
    > Bằng cách áp dụng **State Pattern** cài đặt ở [state.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/state.py), chúng em đã đóng gói hành vi của mỗi trạng thái vào một lớp độc lập (Ví dụ: `PendingState`, `PaidState`). Mỗi trạng thái tự chịu trách nhiệm quyết định bước tiếp theo là gì. Khi muốn thêm trạng thái mới, chúng em chỉ cần tạo một Class trạng thái mới, tuân thủ nguyên lý Open/Closed Principle."
...
*   **Gợi ý trả lời**:
    ...
    > 2. Nó cho phép định nghĩa các logic duyệt đặc thù (như bỏ qua đơn hàng lỗi, lọc đơn hàng chưa thanh toán trong quá trình duyệt) một cách tập trung ngay tại lớp tập hợp được định nghĩa tại [iterator.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/iterator.py)."
...
*   **Gợi ý trả lời**:
    > "Việc kết hợp đa ngôn ngữ là mô hình phổ biến trong kiến trúc Microservices thực tế để tận dụng thế mạnh của từng nền tảng:
...
*   **Gợi ý trả lời**:
    > "Trong hàm [AuthService.login() ở auth_service.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/services/auth_service.py#L12-L66), luồng xử lý diễn ra như sau:
    ...
    > 3. Nếu xảy ra ngoại lệ kết nối (ví dụ: C# service chưa bật hoặc gặp sự cố mạng), khối lệnh `except Exception` sẽ bắt lỗi này và ghi nhận warning. Ngay lập tức, hệ thống tự động rơi vào khối lệnh **Fallback**: thực hiện câu lệnh truy vấn SQL trực tiếp xuống bảng `users` trong SQLite cục bộ thông qua Singleton. Nếu đúng thông tin, người dùng vẫn đăng nhập thành công. Nhờ vậy, sự cố của dịch vụ xác thực C# không làm tê liệt toàn bộ hệ thống Web."
...
*   **Gợi ý trả lời**:
    > "Mặc định, thư viện `sqlite3` trong Python ngăn cản việc chia sẻ đối tượng kết nối giữa các luồng khác nhau để tránh xung đột dữ liệu. Tuy nhiên, FastAPI hoạt động theo cơ chế bất đồng bộ và đa luồng, mỗi request có thể chạy trên một luồng khác nhau. Do đó, chúng em phải cấu hình `check_same_thread=False` tại [singleton.py#L29](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/singleton.py#L29) để cho phép instance Singleton duy nhất được gọi từ bất kỳ luồng request nào.
    >
...
*   **Gợi ý trả lời**:
    ...
    > *   **Tầng Controller** ([api_router.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/controllers/api_router.py)): Chỉ làm nhiệm vụ tiếp nhận yêu cầu HTTP, giải nén dữ liệu và gọi dịch vụ, không chứa bất kỳ dòng logic nghiệp vụ hay câu lệnh SQL nào.
    > *   **Tầng Service** ([auth_service.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/services/auth_service.py)): Chỉ chứa logic xử lý nghiệp vụ xác thực và điều phối dữ liệu.
    > *   **DatabaseConnection**: Chỉ có một trách nhiệm duy nhất là quản lý kết nối và thực thi các truy vấn thô xuống SQLite.
    > *   **Các State Classes** ([state.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/state.py)): Mỗi class chỉ xử lý hành vi chuyển dịch trạng thái của chính nó."
...
*   **Gợi ý trả lời**:
    ...
    >
    > Để Container có thể kết nối vượt ra ngoài môi trường ảo và gọi tới các cổng dịch vụ đang lắng nghe trên máy chủ Windows host, Docker cung cấp một DNS đặc biệt là **`host.docker.internal`** cài đặt ở [auth_service.py#L19](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/services/auth_service.py#L19). DNS này sẽ tự động phân giải thành địa chỉ IP của card mạng bridge trên máy host, giúp kết nối mạng thông suốt."
...
*   **Gợi ý trả lời**:
    ...
    >
    > Trong dự án này, chúng em kết hợp cả hai để tận dụng ưu điểm của cả hai mô hình: Tầng Web chính sử dụng **n-Tier (MVC)** giúp quy tụ luồng điều hướng giao diện tập trung, rõ ràng và dễ phát triển nhanh. Các nghiệp vụ phụ trợ cần tính độc lập, bảo mật cao và có khả năng scale riêng biệt (như SSO đăng nhập hệ thống ở [SSOService](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/microservices_csharp/SSOService/Program.cs), kết xuất báo cáo ở [ReportService](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/microservices_csharp/ReportService/Program.cs)) được tách thành các **C# Microservices** chạy cổng riêng."
...
*   **Gợi ý trả lời**:
    > "Lớp `OrderContext` duy trì trạng thái của đơn hàng trên bộ nhớ RAM khi quy trình đang chạy. Để lưu bền vững trạng thái này xuống SQLite, tại bước cuối cùng của quy trình đặt hàng tại [api_router.py#L39-L42](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/controllers/api_router.py#L39-L42) (sau khi Facade chạy xong và trả về trạng thái cuối cùng là `Shipped` từ Context), hệ thống sẽ thực thi câu lệnh SQL:
...
*   **Gợi ý trả lời**:
    ...
    > 1. **Factory Method Pattern** ở [factory.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/factory.py): Lớp `OrderFactory` và cấu trúc lớp `Order` đóng với việc sửa đổi. Khi thêm loại đơn hàng mới, chúng em chỉ tạo lớp mới và thêm một dòng trong factory, hoàn toàn không sửa đổi cấu trúc của Controller hay Facade.
    > 2. **State Pattern** ở [state.py](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/web_mvc/app/patterns/state.py): Cấu trúc chuyển đổi trạng thái đóng với việc sửa đổi logic cũ. Thêm trạng thái mới chỉ yêu cầu tạo Class trạng thái mới kế thừa từ `OrderState` mà không sửa đổi các Class trạng thái hiện có."
...
*   **Gợi ý trả lời**:
    ...
    > 1. **Tách biệt phân vùng mạng**: Web API đóng vai trò là cửa ngõ duy nhất giao tiếp với người dùng và Proxy các cuộc gọi. Cụm Microservices phía sau có thể cấu hình ẩn trong mạng nội bộ, chỉ chấp nhận request từ IP của Web Component.
    > 2. **Xác thực**: C# [SSOService](file:///d:/HSU/2533Semester%203(2025-2026)/Ki%E1%BA%BFn%20tr%C3%BAc%20ph%E1%BA%A7n%20m%E1%BB%81m/Kien_Truc_Phan_Mem_Test-main/Kien_Truc_Phan_Mem_Test-main/Project/src/microservices_csharp/SSOService/Program.cs) cấp token bảo mật khi đăng nhập thành công.
    > 3. **SQL Parameterization**: Mọi câu lệnh SQL truy vấn SQLite (cả ở Python và C#) đều sử dụng tham số hóa dạng `?` hoặc `@param` để ngăn chặn hoàn toàn lỗi bảo mật nghiêm trọng **SQL Injection**."
