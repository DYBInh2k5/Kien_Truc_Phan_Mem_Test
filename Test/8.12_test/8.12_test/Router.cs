...
{
    /// <summary>
    /// ?óng vai trò là b? ??nh tuy?n (Router) chính c?a Web Server.
    /// Quy?t ??nh n?i dung HTML tr? v? d?a trên HTTP Method (GET/POST) và URL Path.
    /// Qu?n lý danh sách phòng chat và x? lý logic xác th?c ng??i dùng.
    /// </summary>
    class Router
    {
        private static Dictionary<string, string> sessionStore = new Dictionary<string, string>(); // token -> username
        ...
        // Trang Home hi?n th? thông tin sinh viên và liên k?t t?i trang Login
            ...
            <body>
                <div class='card'>
                    ...
                    <div class='info-box'>
                        ...
                        <p><strong>Full Name:</strong> Võ Duy Bình</p>
        ...
        private static void HandleLogin(Socket staff, string body)
        {
            ...
            if (authenticatedUser != null)
            {
                ...
                sessionStore[token] = authenticatedUser.username; // L?u username vào session
        ...
        private static void HandleGetChatRoom(Socket staff, string path)
        {
            ...
            if (room != null)
            {
                room.IsOnline(); // C?p nh?t tr?ng thái
                var builder = new ChatBuilder();
                builder.setTitle(room.Name);
                
                // S? d?ng addMessage c?a Builder thay vì c?ng chu?i th? công
        ...
        /// <summary>
        /// X? lý POST /chat/{id}
        /// - L?y n?i dung message t? body, decode và thêm vào danh sách tin nh?n c?a phòng
        ...
        private static void HandlePostChatRoom(Socket staff, string path, string body, string cookie)
        {
            ...
            if (room != null && !string.IsNullOrEmpty(messageContent))
            {
                ...
                if (!string.IsNullOrEmpty(cookie))
                {
                    ...
                    {
                        var kv = c.Trim().Split('=');
                        if (kv.Length == 2 && kv[0] == "token" && sessionStore.ContainsKey(kv[1]))
                        {
                            user = sessionStore[kv[1]];
                            break;
                        }
                    }
                }
