...
namespace _8._12_test
{
    ...
    internal class JsonAccountSingleton
    {
        ...
        {
            // Thi?t l?p các ???ng d?n kh? thi ?? tìm file accounts.json
            string baseDir = AppDomain.CurrentDomain.BaseDirectory;
            string filePath = Path.Combine(baseDir, "data", "accounts.json");
            
            // N?u không th?y trong bin, th? tìm ? project source (dành cho Visual Studio Debug)
            if (!File.Exists(filePath))
            {
                // ?i ng??c lên 3 c?p t? bin/Debug/net8.0/ ?? vào th? m?c project
                string projectDir = Path.GetFullPath(Path.Combine(baseDir, "..", "..", ".."));
                filePath = Path.Combine(projectDir, "data", "accounts.json");
            }

            // Ki?m tra và n?p d? li?u
            if (File.Exists(filePath))
            {
