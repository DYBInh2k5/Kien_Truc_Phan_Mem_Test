...
pip install -r service3_gateway\requirements.txt
...
Write-Host "5. Starting Service 2 (Order Service - Port 8002) in background..."
$env:USER_SERVICE_URL="localhost:50051"
Start-Process -NoNewWindow -FilePath ".\venv\Scripts\python.exe" -ArgumentList "service2_order\main.py"

Write-Host "Waiting 3 seconds for backend services to start..."
Start-Sleep -Seconds 3

Write-Host "6. Starting Service 3 (API Gateway - Port 8000). Press Ctrl+C to stop..."
$env:ORDER_SERVICE_URL="http://localhost:8002"
# Force Python encoding to handle UTF8 prints properly in Windows CMD
$env:PYTHONUTF8=1
python service3_gateway\main.py
